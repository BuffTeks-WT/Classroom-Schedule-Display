...
def run_tests():
    ...
    try:
        ...
        logger.info(f"Attempting to create reservation for host: {test_host_name}")
        ...
        logger.info(f"Searching for reservations by host name: '{test_host_name}'...")
        ...
        if not found:
            logger.error("Could not find the new reservation by unique host name!")
            ...
            if results_by_id:
                ...
                logger.warning("BUG HINT: Found by ID but NOT by Host Name filter!")
            else:
                logger.error("Could not find reservation by ID either. Creation might have rolled back?")
                return

        # --- Update ---
        log_step(4, "Updating Reservation")
        new_title = "UPDATED Test Event Title"
        logger.info(f"Updating status to 'Confirmed' and title to '{new_title}'...")
        ...
        if success:
            ...
            if updated_res and updated_res[0]['event_title'] == new_title:
                logger.info("Verification: Title was successfully updated in DB.")
            else:
                logger.error("Verification Failed! Data in DB does not match update.")
        else:
        ...
        if del_success:
            ...
            if not remaining:
                ...
                logger.error("Verification Failed! Reservation still exists in DB.")
        else:
